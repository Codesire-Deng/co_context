...
class lazy_nonblocking_awaiter : public lazy_awaiter {
  protected:
    ...
    bool is_ready() const noexcept {
        return !((result_ == -EAGAIN) | (result_ == -EWOULDBLOCK));
    }

    explicit lazy_nonblocking_awaiter(int fd) noexcept : fd_(fd) {}

  protected:
    epoll_fd_meta io_info_;
    int result_;
    int fd_;
};
...
struct lazy_readv
    ...
    inline
    lazy_readv(int fd, std::span<const iovec> iovecs, uint64_t offset) noexcept
    ...
    [[nodiscard]]
    int do_syscall() const noexcept {
        return ::preadv(fd_, iovecs.data(), iovecs.size(), offset);
    }
...
struct lazy_readv2
    ...
    [[nodiscard]]
    int do_syscall() const noexcept {
        return ::preadv2(fd_, iovecs.data(), iovecs.size(), offset, flags);
    }
...
struct lazy_writev
    ...
    inline
    lazy_writev(int fd, std::span<const iovec> iovecs, uint64_t offset) noexcept
    ...
    [[nodiscard]]
    int do_syscall() const noexcept {
        return ::pwritev(fd_, iovecs.data(), iovecs.size(), offset);
    }
...
struct lazy_writev2
    ...
    [[nodiscard]]
    int do_syscall() const noexcept {
        return ::pwritev2(fd_, iovecs.data(), iovecs.size(), offset, flags);
    }
...
struct lazy_accept
    ...
    inline
    lazy_accept(int fd, sockaddr *addr, socklen_t *addrlen, int flags) noexcept
        : lazy_nonblocking_read_awaiter(fd)
        , addr(addr)
        , addrlen(addrlen)
        , flags(flags) {}
    ...
    [[nodiscard]]
    int do_syscall() const noexcept {
        return ::accept4(fd_, addr, addrlen, flags | SOCK_NONBLOCK);
    }
...
struct lazy_read
    ...
    int do_syscall() const noexcept {
        if (offset == -1ULL) {
            ...
            return ::pread(fd_, buf.data(), buf.size(), offset);
        }
    }
...
struct lazy_write
    ...
    int do_syscall() const noexcept {
        if (offset == -1ULL) {
            ...
            return ::pwrite(fd_, buf.data(), buf.size(), offset);
        }
    }
...
struct lazy_send
    ...
    [[nodiscard]]
    int do_syscall() const noexcept {
        return ::send(fd_, buf.data(), buf.size(), flags);
    }
