...
task<void> prep_file_contents(
    ...
    if (ret < file_size) {
        (void)fprintf(stderr, "Encountered a short read.\n");
    }
 ...
 * */
size_t prep_headers(
    const std::filesystem::path &path, off_t len, std::span<char> send_buffer
) {
     ...
     * */
    const char *file_ext = get_filename_ext(path.c_str());
    if (strcmp("jpg", file_ext) == 0 || strcmp("jpeg", file_ext) == 0) {
    ...
    } else if (strcmp("htm", file_ext) == 0 || strcmp("html", file_ext) == 0) {
...
task<> session(const int sockfd) {
    ...
    size_t nr = co_await sock.recv({recv_buf + 6, sizeof(recv_buf) - 6});

    std::span<char> head = get_line({recv_buf + 6, nr});
    ...
    const size_t n_header =
        prep_headers(filepath, path_stat.st_size, header_buf);

    // get file_content prepared
    std::vector<char> content_buf{};
    content_buf.resize(path_stat.st_size);
    co_await prep_file_contents(filepath, path_stat.st_size, content_buf);
